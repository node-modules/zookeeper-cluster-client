...
class ZookeeperAPIClient extends APIClientBase {
  ...
  async getData(path, watcher) {
    if (watcher) {
      return await this._invokeWithWatcher('getData', [ path ], watcher);
    }
    return await this._invokeWithoutWatcher('getData', [ path ]);
  }

  // async exists(path, [watcher])
  async exists(path, watcher) {
    if (watcher) {
      return await this._invokeWithWatcher('exists', [ path ], watcher);
    }
    return await this._invokeWithoutWatcher('exists', [ path ]);
  }

  // async getChildren(path, [watcher])
  async getChildren(path, watcher) {
    if (watcher) {
      return await this._invokeWithWatcher('getChildren', [ path ], watcher);
    }
    return await this._invokeWithoutWatcher('getChildren', [ path ]);
  }
  ...
  async setData(path, data, version) {
    ...
    return await this._invokeWithoutWatcher('setData', args);
  }
  ...
  async create(path, data, acls, mode) {
    ...
    return await this._invokeWithoutWatcher('create', args);
  }
  ...
  async mkdirp(path, data, acls, mode) {
    ...
    return await this._invokeWithoutWatcher('mkdirp', args);
  }
  ...
  async remove(path, version) {
    ...
    return await this._invokeWithoutWatcher('remove', args);
  }

  async getACL(path) {
    return await this._invokeWithoutWatcher('getACL', [ path ]);
  }
  ...
  async setACL(path, acls, version) {
    ...
    return await this._invokeWithoutWatcher('getChildren', args);
  }
  ...
  _invokeWithWatcher(method, args, watcher, withStat = false) {
    const path = args[0];
    ...
    return new Promise((resolve, reject) => {
      this._client.invokeWithWatcher(this.clientId, key, method, args, (err, response) => {
        ...
        resolve(withStat ? response.callbackArgs : response.callbackArgs[0]);
      });
    });
  }
